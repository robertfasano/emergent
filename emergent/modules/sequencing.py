...
class Sequencer(Thing):
    ...
    def goto(self, step, multiset = False):
        step = self.get_step(step)
        if multiset:
            self.multiset(step.state)
        else:
            for channel in step.state:
                self.parent.switches[channel].set(step.state[channel])

    def get_step(self, step):
        ''' Returns a Timestep object corresponding to the passed integer (place) or string (name). '''
        if type(step) == int:
    ...
    def run(self):
        stream = self.form_stream(dt = 0.2)
        import time
        while True:

...
#                  (info[0], info[1], info[2], ljm.numberToIP(info[3]), info[4], info[5]))
    ...
    def spi_initialize(self, mode = 3, block = 0, CLK=0, CS=1,MOSI=2, MISO=3):  #, CS, CLK, MISO, MOSI):
        ''' Args:
            ...
            int block: the labjack terminal block to use. block=0 uses FIO0-3, block=4 uses FIO4-7, etc
            '''
        """
        ...
        ljm.eWriteName(self.handle, "SPI_CS_DIONUM", CS)  # CS is FIO1
        ljm.eWriteName(self.handle, "SPI_CLK_DIONUM", CLK)  # CLK is FIO0
        ljm.eWriteName(self.handle, "SPI_MISO_DIONUM", MISO)  # MISO is FIO3
        ljm.eWriteName(self.handle, "SPI_MOSI_DIONUM", MOSI)  # MOSI is FIO2
...
#        print(slope/error)
    ...
    def stream_in_out(self, input_channels, output_channel, scanRate, targetRate, stopped = False):
        ''' Stream data in on input_channel, apply processing, and stream out on output_channel '''
        ...
        aNames = ["AIN_ALL_NEGATIVE_CH",
                  "STREAM_SETTLING_US", "STREAM_RESOLUTION_INDEX"]
        aValues = [ljm.constants.GND, 0, 0]
        ljm.eWriteNames(self.handle, len(aNames), aNames, aValues)

        aNames = [x+'_RANGE' for x in input_channels]
        aValues = list(np.ones(len(aNames))*10.0)
        ljm.eWriteNames(self.handle, len(aNames), aNames, aValues)
...
#                data = ret[0]
    ...
    def process_data(self, data):
        ...
        pmt = data[0:int(len(data)/2)]
        slowing = data[int(len(data)/2):len(data)]

        pmt -= np.min(pmt)
        slowing -= np.min(slowing)

        pmt *= np.max(slowing)/np.max(pmt)
        pmt -= slowing
        data = pmt
        return data
...
#    t7.stream_start('AIN0', scanRate = 100000, subtract_DC = True)
    t7.run_thread(target='stream_in_out',args=(['AIN0','AIN1'], 'DAC0', 2000,1))
